...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$error) {
    // Insert into found_items table with user_id
    $stmt = $conn->prepare("INSERT INTO found_items
            (user_id, product_name, product_type, found_date, unique_marks, found_location, description, image_url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
<head>
  ...
  <title><?= $website_name ?> | Report Found</title>
...
<body>
  ...
  <div class="bg-gradient-to-r from-[#0f172a]  to-[#334155] min-h-screen flex items-center justify-center p-6">
    ...
    <div class="bg-white bg-opacity-95 shadow-2xl rounded-xl w-full max-w-3xl p-10">
      <h2 class="text-4xl font-extrabold text-center mb-8 text-gray-800">Report Found Item</h2>
      ...
      <form method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div>
            <label class="text-sm font-medium text-gray-700">Found By</label>
            <input type="text" name="found_by" value="<?= htmlspecialchars($userFullName) ?>" readonly
          ...
          <div>
            <label class="text-sm font-medium text-gray-700">Date Found</label>
        ...
        <div>
          <label class="text-sm font-medium text-gray-700">Found Location</label>
